...
import wirecross.IntersectionPoint;
import wirecross.Point;
import wirecross.WirePath;
import wirecross.lossfunction.CombinedStepsDistance;
import wirecross.lossfunction.LossFunction;
import wirecross.lossfunction.ManhattanDistance;
...
public class Day3 {
    ...
    private static IntersectionPoint intersection(WirePath path1, WirePath path2, LossFunction lf) {
        Set<CuratedPoint> points1 = path1.getPoints();
        Set<CuratedPoint> points2 = path2.getPoints();
        points1.retainAll(points2);
        return points1.stream()
        .map(p -> {
            CuratedPoint ip = points2.stream().filter(p2 -> p2.equals(p)).findFirst().get();
            return new IntersectionPoint(p.getPoint(), lf.apply(p, ip));
        })
                .min((p1, p2) -> (p1.getLoss().compareTo(p2.getLoss())))
                .orElseThrow(() -> new RuntimeException("Paths do not intersect"));
    }
}
